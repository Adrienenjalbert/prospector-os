...
- For coaching, propose ONE highest-leverage theme, not a laundry list.`
  ...
  // Few-shot exemplars from prior thumbs-up turns (A1.1).
  const exemplars = formatExemplars(profile, opts.role ?? 'leader', opts.intentClass ?? 'general_query')
...
export async function buildLeadershipLensPrompt(
  ...
  const parts = await buildLeadershipLensPromptParts(tenantId, ctx, packed, opts)
